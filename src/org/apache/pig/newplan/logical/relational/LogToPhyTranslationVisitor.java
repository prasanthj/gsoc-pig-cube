...
public class LogToPhyTranslationVisitor extends LogicalRelationalNodesVisitor {
	...
	PONative poNative = new PONative(new OperatorKey(scope, nodeGen.getNextNodeId(scope)));
	...
	currentWalker.walk(new ExpToPhyTranslationVisitor(currentWalker.getPlan(), childWalker, filter, currentPlan, logToPhyMap));
	popWalker();
	...
	for (LogicalExpressionPlan plan : logPlans) {
	    ...
	    pushWalker(childWalker);
	    childWalker.walk(new ExpToPhyTranslationVisitor(currentWalker.getPlan(), childWalker, sort, currentPlan, logToPhyMap));
	...
	if (sort.getUserFunc() == null) {
	    poSort = new POSort(new OperatorKey(scope, nodeGen.getNextNodeId(scope)), sort.getRequestedParallelism(), null, sortPlans, sort.getAscendingCols(), null);
	} else {
	    POUserComparisonFunc comparator = new POUserComparisonFunc(new OperatorKey(scope, nodeGen.getNextNodeId(scope)), sort.getRequestedParallelism(), null, sort.getUserFunc());
	    poSort = new POSort(new OperatorKey(scope, nodeGen.getNextNodeId(scope)), sort.getRequestedParallelism(), null, sortPlans, sort.getAscendingCols(), comparator);
	}
	...
	if (cross.isNested()) {
	    ...
	    POGlobalRearrange poGlobal = new POGlobalRearrange(new OperatorKey(scope, nodeGen.getNextNodeId(scope)), cross.getRequestedParallelism());
	    poGlobal.addOriginalLocation(cross.getAlias(), cross.getLocation());
	    POPackage poPackage = new POPackage(new OperatorKey(scope, nodeGen.getNextNodeId(scope)), cross.getRequestedParallelism());
	    poGlobal.addOriginalLocation(cross.getAlias(), cross.getLocation());
		...
		for (Operator op : inputs) {
		    ...
		    POLocalRearrange physOp = new POLocalRearrange(new OperatorKey(scope, nodeGen.getNextNodeId(scope)), cross.getRequestedParallelism());
	...
	String scope = DEFAULT_SCOPE;
	POStream poStream = new POStream(new OperatorKey(scope, nodeGen.getNextNodeId(scope)), stream.getExecutableManager(), stream.getStreamingCommand(), this.pc.getProperties());
	...
	POProject exprOp = new POProject(new OperatorKey(scope, nodeGen.getNextNodeId(scope)));
	...
	for (int i = 0; i < exps.size(); i++) {
	    ...
	    pushWalker(childWalker);
	    childWalker.walk(new ExpToPhyTranslationVisitor(exps.get(i), childWalker, gen, currentPlan, logToPhyMap));
	    popWalker();
	...
	POForEach poFE = new POForEach(new OperatorKey(scope, nodeGen.getNextNodeId(scope)), foreach.getRequestedParallelism(), innerPlans, flattenList);
	...
	for (LogicalExpressionPlan lp : plans) {
	    ...
	    // ExpressionPlan
	    currentWalker.walk(new ExpToPhyTranslationVisitor(currentWalker.getPlan(), childWalker, loj, currentPlan, logToPhyMap));
	...
	POCollectedGroup physOp = new POCollectedGroup(new OperatorKey(DEFAULT_SCOPE, nodeGen.getNextNodeId(DEFAULT_SCOPE)));
	...
	for (int i = 0; i < inputs.size(); i++) {
	    ...
	    poInnerLR.setKeyType(plans.size() > 1 ? DataType.TUPLE : exprPlans.get(0).getLeaves().get(0).getResultType());
	    poInnerLR.setResultType(DataType.TUPLE);
	}

	POMergeCogroup poCogrp = new POMergeCogroup(new OperatorKey(DEFAULT_SCOPE, nodeGen.getNextNodeId(DEFAULT_SCOPE)), inpPOs, innerLRs, relationalOp.getRequestedParallelism());
	return poCogrp;
    }
	...
	if (loj.getJoinType() == LOJoin.JOINTYPE.SKEWED) {
	    POSkewedJoin skj;
	    try {
		skj = new POSkewedJoin(new OperatorKey(scope, nodeGen.getNextNodeId(scope)), parallel, inp, innerFlags);
		...
		if (isLeftOuter) {
			...
			if (inputSchema == null) {
			    int errCode = 1109;
			    String msg = "Input (" + ((LogicalRelationalOperator) inputs.get(1)).getAlias() + ") " + "on which outer join is desired should have a valid schema";
		...
		pfrj = new POFRJoin(new OperatorKey(scope, nodeGen.getNextNodeId(scope)), parallel, inp, ppLists, keyTypes, null, fragment, isLeftOuter, nullTuple, inputSchemas, keySchemas);
	...
	} else if ((loj.getJoinType() == LOJoin.JOINTYPE.MERGE || loj.getJoinType() == LOJoin.JOINTYPE.MERGESPARSE) && (new MapSideMergeValidator().validateMapSideMerge(inputs, loj.getPlan()))) {
		...
		try {
		    smj = new POMergeJoin(new OperatorKey(scope, nodeGen.getNextNodeId(scope)), parallel, inp, joinPlans, keyTypes, loj.getJoinType(), leftSchema, rightSchema, mergedSchema);
		} catch (PlanException e) {
		    int errCode = 2042;
    ...
    private POPackage compileToLR_GR_PackTrio(LogicalRelationalOperator relationalOp, String customPartitioner, boolean[] innerFlags, MultiMap<Integer, LogicalExpressionPlan> innerPlans)
	    throws FrontendException {

	POGlobalRearrange poGlobal = new POGlobalRearrange(new OperatorKey(DEFAULT_SCOPE, nodeGen.getNextNodeId(DEFAULT_SCOPE)), relationalOp.getRequestedParallelism());
	...
	POPackage poPackage = new POPackage(new OperatorKey(DEFAULT_SCOPE, nodeGen.getNextNodeId(DEFAULT_SCOPE)), relationalOp.getRequestedParallelism());
	...
	for (int i = 0; i < inputs.size(); i++) {
	    Operator op = inputs.get(i);
	    List<LogicalExpressionPlan> plans = innerPlans.get(i);
	    POLocalRearrange physOp = new POLocalRearrange(new OperatorKey(DEFAULT_SCOPE, nodeGen.getNextNodeId(DEFAULT_SCOPE)), relationalOp.getRequestedParallelism());
	...
	try {
	    for (int i = 0; i < inputs.size(); i++) {
		PhysicalPlan fep1 = new PhysicalPlan();
		POProject feproj1 = new POProject(new OperatorKey(scope, nodeGen.getNextNodeId(scope)), parallel, i + 1); // i+1
															  // since
															  // the
															  // first
															  // column
	    ...
	    fe = new POForEach(new OperatorKey(scope, nodeGen.getNextNodeId(scope)), parallel, fePlans, flattenLst);
	...
	// TODO for time being set this flag
	boolean isHolistic = false;
	String scope = DEFAULT_SCOPE;
	POCube poCube = new POCube(new OperatorKey(scope, nodeGen.getNextNodeId(scope)), loCube.getRequestedParallelism());
    ...
    private List<Tuple> computeOverallCubeLattice(List<String> operations, MultiMap<Integer, String> dimensions) throws LogicalToPhysicalTranslatorException {
    ...
    // as an optimization algebraic attribute finding is also added to this
    // function
    // this function will also try to determine the algebraic attribute if
    // LOCube.algebraicAttr
    // is null.
    // TODO throw error if algebraic attribute could not be determined
	...
	while (succ != null) {
		...
		while (iter.hasNext()) {
			...
			for (int i = 0; i < oplans.size(); i++) {
				...
				if (source instanceof UserFuncExpression) {
					...
					try {
					    ...
					    // if LODistinct operator appeared
					    // before LOGenerate while iterating
					    // then
					    // distalias would not be null
					    if (distalias != null) {
			...
			// LODistinct operator came before LOGenerate while
			// iterating or UserFuncExpr doesn't contain COUNT as
			// UDF
			if (countalias == null) {
	    ...
	    // TODO do I need to proceed further even after detecting holistic?
	    // descend down the plan
	    try {
    ...
    // TODO add some comments of what this function does
		...
		while (iter.hasNext()) {
			...
			for (int i = 0; i < oplans.size(); i++) {
				...
				if (source instanceof UserFuncExpression) {
					...
					// first 2 argument will be
					// ConstantExpression
					// last argument will be
					// ProjectExpression
	...
	String scope = DEFAULT_SCOPE;
	POLimit poLimit = new POLimit(new OperatorKey(scope, nodeGen.getNextNodeId(scope)), loLimit.getRequestedParallelism());
	...
	if (loLimit.getLimitPlan() != null) {
	    ...
	    pushWalker(childWalker);
	    currentWalker.walk(new ExpToPhyTranslationVisitor(currentWalker.getPlan(), childWalker, loLimit, currentPlan, logToPhyMap));
	...
	String scope = DEFAULT_SCOPE;
	POSplit physOp = new POSplit(new OperatorKey(scope, nodeGen.getNextNodeId(scope)), loSplit.getRequestedParallelism());
	...
	String scope = DEFAULT_SCOPE;
	// System.err.println("Entering Filter");
	POFilter poFilter = new POFilter(new OperatorKey(scope, nodeGen.getNextNodeId(scope)), loSplitOutput.getRequestedParallelism());
	...
	pushWalker(childWalker);
	// currentWalker.walk(this);
	currentWalker.walk(new ExpToPhyTranslationVisitor(currentWalker.getPlan(), childWalker, loSplitOutput, currentPlan, logToPhyMap));
	popWalker();
