...
public class LogicalPlanBuilder {
	...
	alias = buildOp(loc, op, alias, inputAlias, null); // it should actually
							   // return same alias
	try {
	...
	for (Operator losplitoutput : plan.getSuccessors(losplit)) {
		...
		if (OptimizerUtils.planHasNonDeterministicUdf(fragment))
		    throw new ParserValidationException(intStream, loc, new FrontendException(op, "Can not use Otherwise in Split with an expression containing a @Nondeterministic UDF", 1131));
	    } catch (FrontendException e) {
		e.printStackTrace();
	    ...
	    try {
		// get the root expression of the filter plan in LOSplitOutput
		// and copy it
	    ...
	    if (root == null)
		throw new PlanGenerationFailureException(intStream, loc, new FrontendException(op, "Could not retrieve LogicalExpression for LOSplitOutput " + losplitoutput, 2048));
	    if (currentExpr == null)
		currentExpr = root;
	    else
    ...
    String buildJoinOp(SourceLocation loc, LOJoin op, String alias, List<String> inputAliases, MultiMap<Integer, LogicalExpressionPlan> joinPlans, JOINTYPE jt, List<Boolean> innerFlags,
	...
	if (jt == JOINTYPE.SKEWED) {
	    if (partitioner != null) {
		throw new ParserValidationException(intStream, loc, "Custom Partitioner is not supported for skewed join");
	    }

	    if (inputCount != 2) {
		throw new ParserValidationException(intStream, loc, "Skewed join can only be applied for 2-way joins");
	    }
	} else if ((jt == JOINTYPE.MERGE || jt == JOINTYPE.MERGESPARSE) && inputCount != 2) {
	    throw new ParserValidationException(intStream, loc, "Merge join can only be applied for 2-way joins");
	} else if (jt == JOINTYPE.REPLICATED) {
	    if (innerFlags.size() == 2 && innerFlags.get(0) == false) {
		throw new ParserValidationException(intStream, loc, "Replicated join does not support (right|full) outer joins");
	    }
	}
    ...
    String buildCubeOp(SourceLocation loc, LOCube op, String alias, String inputAlias, List<String> operations, MultiMap<Integer, LogicalExpressionPlan> expressionPlans)
    ...
    private void setOpsDimensions(LOCube op, List<String> operations, MultiMap<Integer, LogicalExpressionPlan> expressionPlans) throws FrontendException {
	...
	// List<List<Tuple>> lattice = new ArrayList<List<Tuple>>();
	...
	// List<Operator> inpOpers = op.getPlan().getPredecessors(op);
	...
	for (int operIdx = 0; operIdx < operations.size(); operIdx++) {
		...
		for (int j = 0; j < allExprPlan.size(); j++) {
			...
			if (projExpAlias.lastIndexOf(":") != -1) {
			    projExpAlias = projExpAlias.substring(projExpAlias.lastIndexOf(":") + 1, projExpAlias.length());
			    if (colAlias.equals(projExpAlias) == true) {
				allExprPlan.remove(j);
			    }
			}
	...
	String falias = null;
	try {
	    buildGenerateOp(loc, (LOForEach) foreach, (LOGenerate) gen, operators, allExprPlan, flattenFlags, getUserSchema(allExprPlan));
	...
	try {
	    buildGroupOp(loc, (LOCogroup) groupby, null, inpAliases, exprPlansCopy, GROUPTYPE.REGULAR, innerFlags, null);
	} catch (ParserValidationException pve) {
	    throw new FrontendException(pve);
	}

	// plan.disconnect(op.getPlan().getPredecessors(op).get(0), op);
	plan.add(op);
	plan.connect(groupby, op);

	// cube lattice generation
	// TODO add testcase to it
	// if(lattice.size() == 1) {
	// op.setCubeLattice(lattice.get(0));
	// } else {
	// op.setCubeLattice(generateCombinedCubeLattice(lattice));
	// }

	return op.getAlias();
    }

    // private List<Tuple> generateCombinedCubeLattice(List<List<Tuple>>
    // lattices) {
    // List<Tuple> combinedLattice = new ArrayList<Tuple>();
    // List<Tuple> first = new ArrayList<Tuple>(lattices.get(0));
    //
    // for(int i = 1; i < lattices.size(); i++) {
    // List<Tuple> next = lattices.get(i);
    // for(int m = 0; m < first.size(); m++) {
    // Tuple ftup = first.get(m);
    // for(int n = 0; n < next.size(); n++) {
    // Tuple ntup = next.get(n);
    // combinedLattice.add(combineTuples(ftup, ntup));
    // }
    // }
    // first.clear();
    // first.addAll(combinedLattice);
    // // do not clear during last iteration
    // if( i == lattices.size() - 1 ) {
    // combinedLattice.clear();
    // }
    // }
    // return combinedLattice;
    // }
    //
    ...
    // }
    //
    // private List<Tuple> computeLattice(String op, List<LogicalExpression>
    // lexpList)
    ...
    //
    // for (LogicalExpression lexp : lexpList) {
    // dims.add(((ProjectExpression) lexp).getFieldSchema().alias);
    // }
    //
    // try {
    // if (op.equals(LOCube.CUBE_OP) == true) {
    // result = cd.getLattice(dims);
    // } else if (op.equals(LOCube.ROLLUP_OP) == true) {
    // result = rd.getLattice(dims);
    // }
    // } catch (ExecException e) {
    // throw new FrontendException("Couldn't compute cube lattice.", e);
    // }
    //
    // return result;
    // }
    ...
    String buildGroupOp(SourceLocation loc, LOCogroup op, String alias, List<String> inputAliases, MultiMap<Integer, LogicalExpressionPlan> expressionPlans, GROUPTYPE gt, List<Boolean> innerFlags,
	    String partitioner) throws ParserValidationException {
	if (gt == GROUPTYPE.COLLECTED) {
	    if (inputAliases.size() > 1) {
		throw new ParserValidationException(intStream, loc, "Collected group is only supported for single input");
	    }
		...
		while (it.hasNext()) {
			...
			throw new ParserValidationException(intStream, loc, "Collected group is only supported for columns or star projection");
		    }
		}
	    }
	}
	...
	LoadFunc loFunc;
	try {
	    FuncSpec instantiatedFuncSpec = funcSpec == null ? new FuncSpec(PigStorage.class.getName()) : funcSpec;
	...
	LOLoad op = new LOLoad(loader, schema, plan, ConfigurationUtil.toConfiguration(pigContext.getProperties()), loFunc, alias + "_" + newOperatorKey());
	...
	try {
	    ...
	    if (absolutePath == null) {
		absolutePath = stoFunc.relToAbsPathForStoreLocation(filename, QueryParserUtils.getCurrentDir(pigContext));
		if (absolutePath != null) {
		    QueryParserUtils.setHdfsServers(absolutePath, pigContext);
		}
		fileNameMap.put(fileNameKey, absolutePath);
    ...
    void buildGenerateOp(SourceLocation loc, LOForEach foreach, LOGenerate gen, Map<String, Operator> operators, List<LogicalExpressionPlan> exprPlans, List<Boolean> flattenFlags,
	    List<LogicalSchema> schemas) throws ParserValidationException {
    ...
    /**
     * Process expression plans of LOGenerate and set inputs relation for the
     * ProjectExpression
     * 
     * @param foreach
     * @param lp
     *            Logical plan in which the LOGenerate is in
     * @param plan
     *            One of the output expression of the LOGenerate
     * @param operators
     *            All logical operators in lp;
     * @param inputs
     *            inputs of the LOGenerate
     * @throws FrontendException
     */
    private static void processExpressionPlan(LOForEach foreach, LogicalPlan lp, LogicalExpressionPlan plan, Map<String, Operator> operators, ArrayList<Operator> inputs) throws FrontendException {
	...
	while (it.hasNext()) {
		...
		if (projExpr.isRangeProject()) {
		    ...
		    LOInnerLoad innerLoad = new LOInnerLoad(lp, foreach, new ProjectExpression(projExpr, new LogicalExpressionPlan()));
		    ...
		    if (op != null) {
			// this means the project expression refers to a
			// relation
			...
			// in the input of foreach. Add a LOInnerLoad and use
			// that
			// as input
		    ...
		    // the project expression is referring to column in ForEach
		    // input
		    // using position (eg $1)
    ...
    Operator buildNestedOperatorInput(SourceLocation loc, LogicalPlan innerPlan, LOForEach foreach, Map<String, Operator> operators, LogicalExpression expr) throws NonProjectExpressionException,
    ...
    StreamingCommand buildCommand(SourceLocation loc, String cmd, List<String> shipPaths, List<String> cachePaths, List<HandleSpec> inputHandleSpecs, List<HandleSpec> outputHandleSpecs,
    ...
    /**
     * Build a project expression in foreach inner plan. The only difference
     * here is that the projection can be for an expression alias, for which we
     * will return whatever the expression alias represents.
     * 
     * @throws RecognitionException
     */
    LogicalExpression buildProjectExpr(SourceLocation loc, LogicalExpressionPlan plan, LogicalRelationalOperator op, Map<String, LogicalExpressionPlan> exprPlans, String colAlias, int col)
	    throws RecognitionException {
	...
	if (colAlias != null) {
		...
		// The projected alias is actually expression alias, so the
		// projections in the represented
		// expression doesn't have any operator associated with it. We
		...
		return (LogicalExpression) planCopy.getSources().get(0);// get
									// the
									// root
									// of
									// the
    ...
    /**
     * Build a project expression for a projection present in global plan (not
     * in nested foreach plan).
     * 
     * @throws ParserValidationException
     */
	...
	ProjectExpression result = null;
	result = colAlias != null ? new ProjectExpression(plan, input, colAlias, relOp) : new ProjectExpression(plan, input, col, relOp);
	result.setLocation(loc);
	return result;
    }
     ...
     * @param input
     * @param startExpr
     *            the first expression to be projected, null if everything from
     *            first is to be projected
     * @param endExpr
     *            the last expression to be projected, null if everything to the
     *            end is to be projected
     * @return project expression
     * @throws ParserValidationException
     */
    LogicalExpression buildRangeProjectExpr(SourceLocation loc, LogicalExpressionPlan plan, LogicalRelationalOperator relOp, int input, LogicalExpression startExpr, LogicalExpression endExpr)
	    throws ParserValidationException {
	...
	if (funcSpec == null) {
	    ...
	    // this point is only reached if there was no DEFINE statement for
	    // funcName
    ...
    Operator buildNestedProjectOp(SourceLocation loc, LogicalPlan innerPlan, LOForEach foreach, Map<String, Operator> operators, String alias, ProjectExpression projExpr,
	    List<LogicalExpressionPlan> exprPlans) throws ParserValidationException {
	Operator input = null;
	String colAlias = projExpr.getColAlias();
	if (colAlias != null) {
	    // ProjExpr refers to a name, which can be an alias for another
	    // operator or col name.
	    Operator op = operators.get(colAlias);
	    if (op != null) {
		// ProjExpr refers to an operator alias.
		input = op;
	    } else {
		// Assuming that ProjExpr refers to a column by name. Create an
		// LOInnerLoad
	...
	for (LogicalExpressionPlan plan : exprPlans) {
	    ...
	    LOInnerLoad iload = (al == null) ? new LOInnerLoad(lp, f, pe.getColNum()) : createInnerLoad(loc, lp, f, al);
	...
	if (modifier.equalsIgnoreCase("collected")) {
	    ...
	    throw new ParserValidationException(intStream, loc, "Only COLLECTED, REGULAR or MERGE are valid GROUP modifiers.");
	}
    }
	...
	if (modifier.equalsIgnoreCase("repl") || modifier.equalsIgnoreCase("replicated")) {
	    ...
	    throw new ParserValidationException(intStream, loc, "Only REPL, REPLICATED, HASH, SKEWED, MERGE, and MERGE-SPARSE are vaild JOIN modifiers.");
	}
    }
